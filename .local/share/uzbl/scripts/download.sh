...
case "$suggested_filename" in
    ...
    #    ;;
    # luke torrent dans ~/watch
    *.torrent)
        path="/home/luke/watch/$suggested_filename"
        ;;
    # Default case
    *)
...
notify-send " Download " " $path.$count begin... " -i /usr/share/icons/HighContrast/32x32/devices/drive-harddisk-ieee1394.png && \
echo "$path.$count" && \
notify-send " Download " " $path.$count END ! " -i /usr/share/icons/HighContrast/32x32/devices/drive-harddisk-ieee1394.png
